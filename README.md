...
* The modpack features [e4mc](https://modrinth.com/mod/e4mc), which gives users the ability to let their friends connect to their world through the internet, without the use of additional software or mods on the friend side.
  ...
  * A proxy server in one of the four locations in the world is used to generate a randomized "IP" (actually a subdomain). The host's own IP address is never shared.
  ...
  * Both LAN and Internet variants use the same world settings (gamemode, cheats enabled, etc.), including the 8-player limit.
